...
public class UserRepository extends JDBConnection {
	...
	public int insert(User user) {
		String sql = " INSERT INTO user (id, password, name, gender, birth, mail, phone, address, regist_day) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ";
		int result = 0;
	...
	public int update(User user) {
		String sql = " UPDATE user SET id = ?, name = ?, gender = ?, birth = ?, mail = ?, phone = ?, adress = ?) VALUES (?, ?, ?, ?, ?, ?, ?) ";
		int result = 0;
		try {
			psmt = con.prepareStatement(sql);
